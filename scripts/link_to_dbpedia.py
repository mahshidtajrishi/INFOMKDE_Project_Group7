...
def search_dbpedia(ingredient_name):
    """
    Search DBpedia for an ingredient using the Lookup API.
    Returns the DBpedia URI if found, None otherwise.
    """
...
def search_dbpedia_sparql(ingredient_name):
    """
    Alternative: Search DBpedia using SPARQL endpoint.
    """
    clean_name = clean_ingredient_name(ingredient_name)
    if not clean_name or len(clean_name) < 2:
        return None
...
def search_wikidata(ingredient_name):
    """
    Search Wikidata for an ingredient using the API.
    Returns the Wikidata entity URI if found.
    """
    clean_name = clean_ingredient_name(ingredient_name)
    if not clean_name or len(clean_name) < 2:
        return None
...
def get_dbpedia_info(dbpedia_uri):
    """
    Fetch additional information from DBpedia about an entity.
    """
    sparql_endpoint = "http://dbpedia.org/sparql"
